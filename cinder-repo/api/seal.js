...
import { db, tok, readBody, DAY, MAX_CT_CHARS, MAX_VIEWS, MAX_TTL } from './_lib.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    await db().set('s:' + id, JSON.stringify(rec), { ex: exp });
    await db().set('r:' + receiptToken, JSON.stringify({
